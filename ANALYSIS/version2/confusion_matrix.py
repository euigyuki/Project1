...
import json
import yaml
from fleiss_kappa import load_config
...
def plot_confusion_matrix_indoor_outdoor(
    ...
    plt.title("version 2 Indoor/Outdoor Confusion Matrix")
...
def category(images_or_captions):
    param1 = "category"
    filename = f"{images_or_captions}2.csv"
    df = pd.read_csv(filename)

    answers_1 = df[df["WorkerId"] == worker_id_1]["Answer.taskAnswers"]
    answers_2 = df[df["WorkerId"] == worker_id_2]["Answer.taskAnswers"]

    save_path = f"{path}/{images_or_captions}_category_confusion_matrix.png"

    derrick_categories = get_category_labels(answers_1, param1)
    ken_categories = get_category_labels(answers_2, param1)
...
def plot_confusion_matrix_manmade_natural(
    ...
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_df, annot=True, fmt="d", cmap="Blues")
    plt.title("Version 2 Man-made/Natural Confusion Matrix")
    plt.ylabel(f"{true_name} Prediction")
    plt.xlabel(f"{pred_name} Prediction")
    total_sum = np.sum(cm)
    plt.text(
...
def manmade(images_or_captions):
    ...
    save_path = f"{path}/{images_or_captions}_manmade_natural_confusion_matrix.png"
...
def outdoors(images_or_captions):
    ...
    answers_3 = df[df["WorkerId"] == worker_id_3]["Answer.taskAnswers"]
    print(answers_1.iloc[0])
    save_path = f"{path}/{images_or_captions}_indoor_outdoor_confusion_matrix.png"
    derrick = get_true_labels(answers_1, param1, param2)
    ken = get_true_labels(answers_2, param1, param2)
